...
export default class extends Base {
    ...
    async addmarkdownAction() {
        //获取分类
        let tagsList = await this.model("tags").where({ id: { ">": 0 } }).limit(10, 10).select();
        this.assign("tagsList", tagsList);
        //获取类型
        let itemList = await this.model("item").limit(10).select();
        this.assign("itemList", itemList);
        //新增
    ...
    async delsomeAction() {
        ...
        if (arr) {
            rs = await this.model("article").where({ id: ["IN", arr] }).delete();
        }
        if (rs) {
            //操作成功
            return this.success();
